...
namespace kcp2k
{
    ...
    public class KcpTransport : Transport, PortTransport
    {
        ...
        public override void ChangeNetworkAddress(string newAddress)
        {
            if (!NetworkManager.singleton.IsP2PMode()) return;
            if (!Common.ResolveHostname(newAddress, out IPAddress[] addresses))
            {
                return;
            }
            (client.remoteEndPoint as IPEndPoint).Address = addresses[0];
        }
    }
}
